...
const UNSAFE_PATTERNS = [
  ...
  {
    pattern: /\b(xxx|explicit adult|pornographic|sexual content)\b/i,
  ...
  {
    pattern: /\b(api[_-]?key|secret[_-]?key|access[_-]?token)\s*[=:]\s*['"]?\S{10,}/i,
    reason: 'potential secret or credential',
  },
  ...
  {
    pattern: /\b(credit\s+card|card\s+number)\s*[:\s]\s*\d{4}[- ]?\d{4}[- ]?\d{4}[- ]?\d{4}\b/i,
    reason: 'potential credit card (PII)',
  },
  {
    pattern: /\bemail\s+me\s+at\s+[\w.+-]+@[\w.-]+\.\w+\s+with\s+personal/i,
    reason: 'PII sharing request',
  },
];
...
export function isSafeContent(content: string): SafetyCheckResult {
  ...
  const isTechnicalContext = TECHNICAL_CONTEXT_PATTERNS.some((pattern) => pattern.test(content));
